<template>
  <div class="detail-play-container pr">
    ...
    <div class="play-video-photo" v-show="showPhoto">
      ...
      <template>
        <yxt-icon name="play-circle-o" class="detail_play_icon" v-if="!isShowProgress" @click="getPlayHandler" />
        <div class="detail-play-study" v-if="isShowProgress && kngInfo.schedule === 0">
          <p class="text-white font-size-16">未开始学习</p>
          <yxt-button round size="medium" style="margin-top: 16px;" @click="getPlayHandler">开始学习</yxt-button>
        </div>
        <div class="detail-play-study" v-if="isShowProgress && kngInfo.schedule !== 0">
          <yxt-button round size="medium" @click="getPlayHandler">继续学习</yxt-button>
        </div>
      </template>
    </div>
  </div>
